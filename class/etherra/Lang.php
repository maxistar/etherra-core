...
class Lang {
            ...
            if (is_dir($dir)){
            	if ($dh = opendir($dir)) {
            		while (($file = readdir($dh)) !== false) {
            			...
            			if (!is_dir($dir.'/'.$file.'/lang/')) continue;
            			self::loadFile($dir.'/'.$file.'/lang/',$name, $suffix);
            		}
            		closedir($dh);
            	}
            }
            self::loadFile(_SITE_ROOT.'inc/theme/'.conf('site.theme').'/lang/', $name, $suffix);
            self::loadFile(_SITE_ROOT.'inc/lang/', $name, $suffix);
        }
    }
    ...
    static function plural($word,$count) {
        ...
        if (isset(self::$data[$word][$form])) return sprintf(self::$data[$word][$form],$count);
